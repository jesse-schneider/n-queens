...
def breadth_first_search(Board):
    ...
    while not queue.empty():
        ...
        for i in range(Board.n):
            ...
            new_state = QueensBoard(
                Board.n,  current_row + 1, new_queens, new_coords)

            if (current_row + 1) == Board.n:
                #check goal state
                result = new_state.bfs_check_collisons()

                #PRUNED: if queens are already clashing, there is no way for a branch of this to be a goal state
                if not result:
                    continue
